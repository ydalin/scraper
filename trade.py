# trade.py – SYMBOL-USDT, configurable trailing stop and reduceOnly on all exits
...
async def execute_trade(client, signal, usdt_amount, leverage=10, config=None, dry_run=False):
    ...
    # BingX uses AAA-BBB format for symbols
    ...
    # Entry order (one-way mode → side only, no positionSide)
    ...
    for i, tp_price in enumerate(targets[:4]):
        ...
        tp_payload = {
            ...
            "workingType": "MARK_PRICE",
            "reduceOnly": "true",  # <-- changed from 'false'
        }
