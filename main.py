# main.py – FINAL ×10 BOT – WITH TINY AMOUNTS OPTION FOR SAFE TESTING
import asyncio
import hashlib
...
# Always live API (no virtual API exists)
base_url = "https://open-api.bingx.com"
client_bingx = {'api_key': api_key, 'secret_key': secret_key, 'base_url': base_url}

# TINY AMOUNTS OPTION – ASK EVERY TIME
tiny_mode = input("   Use tiny trades for testing ($1–$9 per trade)? (y/n) [n]: ").strip().lower()
use_tiny = tiny_mode == 'y'
if use_tiny:
    print("   → TINY MODE ENABLED – $1–$9 per trade (real money, ultra-low risk)")
else:
    print("   → NORMAL MODE – 5.8% per trade (full profit mode)")
...
async def get_open_positions_count():
    resp = await bingx_api_request('GET', '/openApi/swap/v2/trade/position', client_bingx['api_key'],
                                   client_bingx['secret_key'])
...
async def main_loop():
    ...
    while True:
        try:
            ...
            if use_tiny:
                usdt_amount = max(1.0, min(9.0, usdt_amount))  # ← FORCES $1–$9
